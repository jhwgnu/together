...
<div id="siDoc"><a name="document_<?php echo $__Context->doc_srl ?>"></a>
	...
	<?php if($__Context->grant->view){ ?><ul class="scFrm">
		<li>
			<?php if($__Context->mi->thumbnail_auto_resize === 'N' || count($__Context->exks)){ ?>
				<?php if($__Context->mi->thumbnail_auto_resize !== 'N'){ ?>
				<?php 
					$__Context->n = (count($__Context->exks)>5?5:count($__Context->exks));
					$__Context->thum_width = (18 * $__Context->n) + (9 * ($__Context->n-1));
					$__Context->thum_height = $__Context->thum_width;
				 ?>
				<?php }else{ ?>
				<?php 
					$__Context->thum_width = $__Context->mi->thumbnail_width;
					$__Context->thum_height = $__Context->mi->thumbnail_height;
				 ?>
				<?php } ?>
				<div class="thmAx clearBar" style="margin-right:<?php echo (int)$__Context->thum_width+25 ?>px;">
					<a class="thum"<?php if($__Context->us_slide&&!$__Context->is_blind&&(!$__Context->is_secret||$__Context->is_granted)){ ?> data-slide="<?php echo $__Context->doc_srl ?>"<?php } ?> href="#" onclick="return false">
					<?php if($__Context->is_blind || ($__Context->is_secret&&!$__Context->is_granted)){ ?>
					<img src="/modules/beluxe/skins/default/img/common/thum_<?php echo $__Context->is_blind?'blind':'secret' ?>.png" alt="<?php echo $__Context->is_blind?'blind':'secret' ?>" title="<?php echo $__Context->is_blind?'blind':'secret' ?>" height="<?php echo $__Context->thum_height ?>" width="<?php echo $__Context->thum_width ?>"/>
					<?php }else{ ?>
					<?php if($__Context->oDocument->thumbnailExists($__Context->mi->thumbnail_width, $__Context->mi->thumbnail_height, $__Context->mi->thumbnail_type)){ ?>
					<img class="thum" src="<?php echo $__Context->oDocument->getThumbnail($__Context->mi->thumbnail_width, $__Context->mi->thumbnail_height, $__Context->mi->thumbnail_type) ?>" border="0" alt="thumbnail" width="<?php echo $__Context->thum_width ?>" />
					<?php }else{ ?>
					<img class="thum" src="/modules/beluxe/skins/default/img/common/thum_no.png" alt="no image" title="no image" height="<?php echo $__Context->thum_height ?>" width="<?php echo $__Context->thum_width ?>" />
					<?php } ?>
					<?php } ?>
					</a>
					<?php if(count($__Context->exks)){ ?><ul class="scVext" style="margin-left:<?php echo (int)$__Context->thum_width+25 ?>px;">
