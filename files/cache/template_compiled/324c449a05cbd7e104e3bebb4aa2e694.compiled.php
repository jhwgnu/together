...
$__tpl=TemplateHandler::getInstance();echo $__tpl->compile('modules/xewall/tpl','header.html') ?>
...
<table id="moduleList" class="x_table x_table-striped x_table-hover">
	<caption>
	...
	<tbody>
		<?php if($__Context->xewall_list&&count($__Context->xewall_list))foreach($__Context->xewall_list as $__Context->no=>$__Context->val){ ?><tr>
...
echo $__Context->module_category[$__Context->val->module_category_srl]->title;
			...
			<td>
				<a href="<?php echo getUrl('act','dispXewallAdminModuleInsert','module_srl',$__Context->val->module_srl) ?>" class="x_icon-cog" title="<?php echo $__Context->lang->cmd_setup ?>"><?php echo $__Context->lang->cmd_setup ?></a>
				<a href="<?php echo getUrl('','module','module','act','dispModuleAdminCopyModule____','module_srl',$__Context->val->module_srl) ?>" class="x_icon-plus" onclick="popopen(this.href);return false;" title="<?php echo $__Context->lang->cmd_copy ?>"><?php echo $__Context->lang->cmd_copy ?></a>
				<a href="<?php echo getUrl('act','dispXewallAdminModuleDelete','module_srl', $__Context->val->module_srl) ?>" class="x_icon-remove" title="<?php echo $__Context->lang->cmd_delete ?>"><?php echo $__Context->lang->cmd_delete ?></a>
			</td>
		</tr><?php } ?>
		<?php if(!$__Context->xewall_list){ ?><tr>
...
if(!in_array($__Context->key, array('mid', 'vid', 'act'))){ ?><input type="hidden" name="<?php echo $__Context->key ?>" value="<?php echo $__Context->val ?>" /><?php }} ?>
	...
	<div class="x_btn-group x_pull-right">
		<a class="x_btn modalAnchor xe-modal-window" href="<?php echo getUrl('act', 'dispXewallAdminInitDB') ?>">DB 초기화</a>
		<a class="x_btn modalAnchor xe-modal-window" href="<?php echo getUrl('act', 'dispXewallAdminModuleInsert') ?>">모듈 생성</a>
	</div>
</div>
