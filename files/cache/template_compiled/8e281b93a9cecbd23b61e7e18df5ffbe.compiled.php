...
<?php if($__Context->oComment->isExists()){ ?><div class="board context_data">
	<h3 class="comment_header">
		<?php if($__Context->oComment->homepage){ ?><a class="author" href="<?php echo $__Context->oComment->homepage ?>"><?php echo $__Context->oComment->getNickName() ?></a><?php } ?>
		<?php if(!$__Context->oComment->homepage){ ?><strong class="author"><?php echo $__Context->oComment->getNickName() ?></strong><?php } ?>
		<span class="time"><?php echo $__Context->oComment->getRegdate('Y.m.d H:i') ?></span>
	</h3>
	<?php echo $__Context->oComment->getContent(false) ?>
</div><?php } ?>
<form action="./" method="get" onsubmit="return procFilter(this, delete_comment)" class="board context_message"><input type="hidden" name="error_return_url" value="<?php echo htmlspecialchars(getRequestUriByServerEnviroment(), ENT_COMPAT | ENT_HTML401, 'UTF-8', false) ?>" /><input type="hidden" name="act" value="<?php echo $__Context->act ?>" /><input type="hidden" name="vid" value="<?php echo $__Context->vid ?>" />
